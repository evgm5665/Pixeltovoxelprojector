...
)
{
    ...
    // We'll obtain a mutable proxy to the voxel grid where needed.
    // Avoid storing pointers to proxy objects which can dangle.
    ...
    // build process addition/change:
    // Iterate over each pixel in the image
    #pragma omp parallel for
    for (pybind11::ssize_t i = 0; i < image_height; ++i)
    {
        // Per-iteration (per-thread) mutable proxy to the voxel grid when provided.
        std::unique_ptr<decltype(voxel_grid.mutable_unchecked<3>())> voxel_grid_mutable_ptr_local;
        if (voxel_grid_provided) {
            voxel_grid_mutable_ptr_local = std::make_unique<decltype(voxel_grid.mutable_unchecked<3>())>(voxel_grid.mutable_unchecked<3>());
        }
        for (pybind11::ssize_t j = 0; j < image_width; ++j)
        {
            ...
            if (brightness > 0)
            {
                ...
                if (voxel_grid_provided)
                {
                    // Use the per-iteration proxy created above
                    auto &voxel_grid_mutable = *voxel_grid_mutable_ptr_local; // Reference to the voxel grid
